...
class GenotypeAnalyzer:
    ...
    def aggregate_genotype_measurement_data(self, geno_list, pos_list, lp_list, mp_list, hp_list):
        def extract_measurement(df_list, sex, rep_label, vial_identifier):
            ts_dict = {}
            for df in df_list:
        ...
        data = {'position': {"M": {}, "F": {}}, 'lp': {"M": {}, "F": {}}, 'mp': {"M": {}, "F": {}}, 'hp': {"M": {}, "F": {}}}
        counters = {"M": {}, "F": {}}
        for geno_df, pos_df, lp_df, mp_df, hp_df in zip(geno_list, pos_list, lp_list, mp_list, hp_list):
            for _, row in geno_df.iterrows():
                ...
                counters[sex][genotype] = counters[sex].get(genotype, 0) + 1
                rep_label = f"{genotype}_rep{counters[sex][genotype]}"
    ...
    def create_plot_aggregated_df(self, df, sex, output_folder, measurement):
        ...
        agg_df.to_csv(os.path.join(output_folder, f"{sex.lower()}_agg_{measurement}_data.csv"), index=False)
        ...
        plt.ylabel(ylabel)
        plt.title(f"{sex.capitalize()} Genotype Aggregated {measurement.capitalize()} Over Time")
        plt.legend()
        plt.grid(True)
        plt.tight_layout()
        # plt.savefig(os.path.join(output_folder, f"{sex.lower()}_plot_{measurement}.png"))
        plt.savefig(os.path.join(output_folder, f"{sex.lower()}_plot_{measurement}.tiff"), dpi=300, format='tiff')
        plt.close()
    ...
    def run(self):
        ...
        for i, sex in enumerate(sexs):
            for measurement in measurements:
                ...
                df.to_csv(os.path.join(out_dir, f"{sex}_stats_{filename_suffix[measurement]}"), index=False)
                self.create_plot_aggregated_df(df, sex, out_dir, measurement)